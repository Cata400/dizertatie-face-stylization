...
def generalized_steps_vgg(x, seq, model, b, content_vgg=None, t_edit=None, t_boost=None, **kwargs):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        x0_preds = []
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            if t_boost is not None and t_boost > i:
                eta = 1
            else:
                eta = 0
            
            if t_edit is not None and i > t_edit:
                et = model(xt, t, content_vgg)
            else:
                et = model(xt, t)        
...
def generalized_steps_vgg_diffstyle(x, seq, model, b, content_vgg=None, t_edit=None, t_boost=None, omega=0):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        x0_preds = []
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            if t_edit is not None and i > t_edit:
                ...
                # et_vgg = model(xt, t, content_vgg)
                # et = model(xt, t)
                
                # new_P_xt = (xt - et_vgg * (1 - at).sqrt()) / at.sqrt()
                ...
                # dP = new_P_xt - P_xt
                # det = et_vgg - et
                # dx = at.sqrt() * dP + omega * (1 - at).sqrt() * det

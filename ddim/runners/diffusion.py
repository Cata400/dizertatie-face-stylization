...
from models.diffusion_vgg import DiffusionVGG, get_content_model_vgg, ProjectorBlock
...
class Diffusion(object):
    ...
    def sample_projector(self):
        ...
        for i, (x, x_vgg) in tqdm.tqdm(enumerate(test_loader), desc="Generating image samples for FID evaluation"):
            ...
            for j in range(self.config.sampling.batch_size):
                tvu.save_image(
                    y[j], os.path.join(self.args.image_folder, f"pred_{i * self.config.sampling.batch_size + j}_projector_good_edit_small.png")
                )   
    ...
    def resample_image(self, x, model, content_vgg=None, last=True):
        ...
        from functions.denoising import generalized_steps_vgg, reverse_generalized_steps
        ...
        if self.args.sample_type == "generalized":
            ...
            x = xs[0][-1]
            x = x.to(self.device)
            xs = generalized_steps_vgg(x, seq, model, self.betas, eta=self.args.eta, content_vgg=content_vgg, t_edit=self.config.diffusion.projector.t_edit)
            x = xs

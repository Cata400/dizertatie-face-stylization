...
def get_content_model_vgg(content_layer):
...
class DiffusionVGG(nn.Module):
    ...
    def forward(self, x, t, content_vgg=None):
        ...
        h_vgg = torch.zeros_like(h).requires_grad_()
    ...
    def setattr_layers(self, nums=1):
        ...
        for i in range(nums):
            setattr(self, f"projector_{i}", ProjectorBlock(in_channels=block_in,
                                    out_channels=block_in,
                                    temb_channels=self.temb_ch,
                                    dropout=0.0))

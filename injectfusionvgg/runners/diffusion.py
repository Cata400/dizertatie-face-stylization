...
class Diffusion(object):
    ...
    def resample_image(self, x, model, content_vgg=None, last=True):
        ...
        if self.args.sample_type == "generalized":
            ...
            #     if i % (1000 // 11) == 0 or i == len(xs[0]) - 1:
            #         img = inverse_data_transform(self.config, xs[0][i])
            #         tvu.save_image(tfs.Resize((1024, 1024))(img[0]), os.path.join(self.args.output, 'forward', f"reconstruct_{str(i).zfill(5)}.png"))
            x = xs[0][-1]
            x = x.to(self.device)
            ...
            #     if i % (1000 // 11) == 0 or i == len(xs[0]) - 1:
            #         img = inverse_data_transform(self.config, xs[0][i])
            #         tvu.save_image(tfs.Resize((1024, 1024))(img[0]), os.path.join(self.args.output, 'reverse', f"reconstruct_{str(i).zfill(5)}.png"))
            x = xs
    ...
    def diffstyle_vgg(self):
        ...
        # style_img_paths = sorted(style_img_paths)
